...
def spikesorting(**kwargs):
    ...
    for electrode in range(iterations):
        ...
        anasig_block = IO.read_block(t_starts=t_starts, t_stops=t_stops, electrode_list=electrode_list, units=None) # TODO: Set this back: electrode_list = [electrode]
        ...
        if block is None:
            ...
            if len(block.segments[0].analogsignalarrays) == 0:
                continue
            recordingchannel = anasig_block.segments[0].analogsignalarrays[0].recordingchannel
        else:
            for s in range(len(block.segments)):
                ...
                for a in range(len(block.segments[s].analogsignalarrays)):
                    ...
                    block.segments[s].analogsignalarrays[a].recordingchannel = recordingchannel
                    block.segments[s].analogsignalarrays[a].recordingchannelgroup = None

        elephant.spike_sorting.generate_spiketrains(block,'phy',
                                                              waveforms=waveforms,
                                                              sort=sort,
                                                              parameter_dict=parameter_dict)
    spikesorting2hdf5(os.path.join(set['sortdir'], session), block)
